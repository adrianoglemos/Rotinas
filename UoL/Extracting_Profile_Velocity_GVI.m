...
prof_shp = shaperead('/nfs/a59/eeagdl/Data/Available_Images/S1/Antarctic/ArcGIS/profile/Profile_GVI.shp');
...
[VEL,Rvel]=geotiffread('/nfs/a59/eeagdl/Data/Available_Images/S1/Antarctic/ArcGIS/arcgis/VEL_mosaic.tif');
VEL(VEL<0)=nan;

[SNR,Rsnr]=geotiffread('/nfs/a59/eeagdl/Data/Available_Images/S1/Antarctic/ArcGIS/arcgis/SNR_mosaic.tif');
SNR(SNR<0)=nan;


[row_vel,col_vel]=map2pix(Rvel,X_p,Y_p);
prof_vel=improfile(VEL,col_vel,row_vel);

[row_snr,col_snr]=map2pix(Rsnr,X_p,Y_p);
prof_snr=improfile(SNR,col_snr,row_snr);

%prof_vel=improfile(VEL,col_vel(~isnan(col_vel)),row_vel(~isnan(row_vel)));



dist=0:(778.637/(length(prof_vel)-1)):778.637;
...
plot(dist,prof_vel,'.'); hold on; 
title('Velocity', 'fontsize',12,'fontweight','b')
...
xlim([0 778]);
ylim([0 2]);
set(gca,'XTick',[0:20:778])
grid on;

figure('units', 'centimeters','position', [0 0 20 15] , 'paperpositionmode', 'auto'); %,'papertype','A4'  maximize;
plot(dist,prof_snr,'.'); hold on; 
title('SNR', 'fontsize',12,'fontweight','b')
%ylabel('km.yr^{-1}','fontsize',12,'fontweight','b')
xlabel('distance [km]','fontsize',12,'fontweight','b')
